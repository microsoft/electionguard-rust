...
//! This module provides the implementation of the inter-Guardian encrypted key share,
//! [`InterguardianShare`].
//!
//! Note that this is a distinct, but related, concept from a `Guardian`'s secret shares,
//! [`z_i`] and [`z_i_hat`].
//!
//! For details, see:
//!
//! - EGDS 2.1.0, Sec. 3.4.4, starting on pg. 25.
...
use util::{bitwise::xor, csrng::Csrng};
...
/*
/// The secret input used to generate a [`InterguardianShare`].
///
/// This object is used in case there is a dispute about the validity of a given [`InterguardianShare`].
#[derive(Serialize, Deserialize)]
pub struct InterguardianShareEncryptionSecret {
...
/*
/// A tuple consisting of a [`InterguardianShare`] and the corresponding [`InterguardianShareEncryptionSecret`].
pub struct InterguardianShareEncryptionResult {
    // The encrypted key shares
    pub ciphertext: InterguardianShare,

    // The corresponding secrets
    pub secret: InterguardianShareEncryptionSecret,
}
// */

/// Represents errors occurring while decrypting a [`InterguardianShare`].
#[derive(thiserror::Error, Clone, Debug, PartialEq, Eq, serde::Serialize)]
pub enum InterguardianShareDecryptionError {
...
/// An inter-Guardian encrypted key share generated by a 'sender' [`Guardian`] for a
/// 'recipient' `Guardian`.
///
/// Note that this is a distinct but related concept from a `Guardian`'s secret shares,
/// [`z_i`] and [`z_i_hat`].
...
pub struct InterguardianShare {
    /// The key being shared.
...
impl InterguardianShare {
    ...
    /*
    /// Computes a [`InterguardianShare`] of a share for a given recipient and nonce.
    ///
    /// The arguments are
    ///
    /// - `election_parameters` - the election parameters
    ...
    /// Creates a new [`InterguardianShareEncryptionResult`] given the sender's secret key for a given recipient.
    ///
    /// The arguments are
    ...
    pub fn encrypt(
        ...
        receiver_public_communication_key: &ElGamalPublicKey,
    ) -> EgResult<InterguardianShareEncryptionResult> {
        Err("TODO rework for EGDS 2.1.0")?
        /*
        ...
        let i = sender_secret_key.guardian_index();
        let l = receiver_public_communication_key.guardian_index();
        ...
        // Encrypt the share
        let ciphertext: InterguardianShare = Self::new(
            election_parameters,
            sender_secret_key.i(),
            &nonce,
            &p_l,
        ...
        let secret = InterguardianShareEncryptionSecret {
        ...
        Ok(InterguardianShareEncryptionResult { ciphertext, secret })
        // */
    }

    /// EGDS 2.1.0 eq. 23 pg. 26.
    ///
    /// Decrypts and validates a [`InterguardianShare`].
    ///
    /// The arguments are
    ///
    /// - `self` - the encrypted key shares
    /// - `election_parameters` - the election parameters
...
/// Errors that may occur with the public validation of a [`InterguardianShare`].
#[derive(thiserror::Error, Clone, Debug, PartialEq, Eq, serde::Serialize)]
pub enum InterGuardianSharePublicValidationError {
    #[error(
        "The inter-Guardian key share is from sender `{expected_dealer}`, but the public key for guardian `{supplied_dealer}` was supplied."
    )]
    ...
    #[error(
        "The inter-Guardian key share is for recipient `{expected_recipient}`, but the public key for guardian `{supplied_recipient}` was supplied."
    )]
    ...
    #[error(
        "The inter-Guardian key share is from sender `{sender}` for recipient `{recipient}`, but the Guardian encryption secret is for sender `{guardian_encryption_secret_dealer}` and recipient `{guardian_encryption_secret_recipient}`."
    )]
    ...
    #[error(
        "The inter-Guardian key share ciphertext does not match the value expected for these values of the sender's public key and the guardian encryption secret and nonce."
    )]
    CiphertextDoesNotMatchExpectedValue,

    #[error("The inter-Guardian key share fails the validity check.")]
    ValidityCheckFailed,
}

/*
impl InterguardianShare {
    /// Validates a [`InterguardianShare`] with respect to given [`InterguardianShareEncryptionSecret`] and [`GuardianPublicKey`] of sender and recipient in case of a dispute.
    ///
    /// The arguments are
    ///
    /// - `self` - the encrypted key shares
    ...
    /// - `receiver_public_key` - the receiver's [`GuardianPublicKey`]
    /// - `secret` - the published [`InterguardianShareEncryptionSecret`]
    ///
    ...
    pub fn public_validation(
        ...
        sender_public_key: &GuardianPublicKey,
        receiver_public_key: &GuardianPublicKey,
        secret: &InterguardianShareEncryptionSecret,
    ) -> EgResult<()> {
        Err("TODO rework for EGDS 2.1.0")?
        /*
        ...
        if self.sender != sender_public_key.i() {
            Err(
                InterguardianSharePublicValidationError::DealerPublicKeyMismatch {
        ...
        if self.recipient != receiver_public_key.i() {
            Err(
                InterguardianSharePublicValidationError::RecipientPublicKeyMismatch {
        ...
        if self.sender != secret.sender {
            Err(
                InterguardianSharePublicValidationError::DealerSecretMismatch {
                    sender: self.sender,
        ...
        if self.recipient != secret.recipient {
            Err(
                InterguardianSharePublicValidationError::RecipientSecretMismatch {
                    sender: self.sender,
                    recipient: self.recipient,
                    guardian_encryption_secret_dealer: secret.sender,
                    guardian_encryption_secret_recipient: secret.recipient,
        ...
        if *self != expected_ciphertext {
            Err(InterguardianSharePublicValidationError::CiphertextDoesNotMatchExpectedValue)?;
        }
        ...
        if g_p_l != rhs {
            Err(InterguardianSharePublicValidationError::ValidityCheckFailed)?;
        }
...
#[derive(thiserror::Error, Clone, Debug, PartialEq, Eq, serde::Serialize)]
pub enum InterGuardianShareGenerationError {
    ...
    /// An [`InterguardianShare`] was missing.
    #[error("The encrypted key shares corresponding to guardian `{0}` is missing.")]
    InterguardianShareMissing(GuardianIndex),

    /// A [`InterguardianShare`] matches the wrong Guardian.
    #[error(
        "The Guardian `{guardian_listed_in_interguardian_share}` listed in the InterGuardianShares share corresponds to the public key from guardian `{guardian_listed_in_public_key}`."
    )]
    InterGuardianShareCorrespondsToWrongPublicKey {
        guardian_listed_in_interguardian_share: GuardianIndex,
        guardian_listed_in_public_key: GuardianIndex,
    },

    /// Different numbers of [`GuardianPublicKey`]s and [`InterguardianShare`]s were supplied.
    #[error(
        "Different numbers of guardian public keys (`{cnt_guardian_public_keys}`) and encrypted key sharess (`{cnt_interguardian_shares}`) were supplied."
    )]
    GuardianPublicKeyAndInterGuardianShareQuantityMismatch {
        cnt_guardian_public_keys: usize,
        cnt_interguardian_shares: usize,
    },

    /// A [`InterguardianShare`] could not be decrypted.
    #[error(
...
impl From<EgError> for InterGuardianShareGenerationError {
    fn from(e: EgError) -> Self {
        InterGuardianShareGenerationError::EgError(Box::new(e))
    }
}

/*
/// A guardian's share of a joint secret key, it corresponds to `P(i)` in Equation `22`. [TODO fix ref]
///
...
impl GuardianSecretKeyShare {
    /// Computes the [`GuardianSecretKeyShare`] from the [`InterguardianShare`]s.
    ///
    /// The arguments are
    ///
    /// - `produce_resource` - common resource data provider
    /// - `guardian_public_keys` - a sequence of [`GuardianPublicKey`]s
    /// - `interguardian_shares` - a sequence of [`InterguardianShare`]
    /// - `receiver_secret_key` - the receiver's [`GuardianSecretKey`]
    ///
    /// Verifies that the i-th `GuardianPublicKey` and the i-th `InterguardianShare` correspond to the same Guardian.
    pub async fn generate(
        produce_resource: &(dyn ProduceResource + Send + Sync + 'static),
        guardian_public_keys: &[&GuardianPublicKey],
        interguardian_shares: &[InterguardianShare],
        receiver_secret_key: &GuardianSecretKey,
        ...
        // Verify that the encrypted key sharess are from every guardian.
        for (guardian_ix0, &guardian_public_key) in guardian_public_keys.iter().enumerate() {
            ...
            let interguardian_share: &InterguardianShare =
                interguardian_shares.get(guardian_ix0).ok_or(
                    GuardianSecretKeyShareGenerationError::InterGuardianShareMissing(
                        guardian_listed_in_public_key,
                    ),
                )?;
            let guardian_listed_in_interguardian_share = interguardian_share.sender;
            if guardian_listed_in_interguardian_share != guardian_listed_in_public_key {
                return Err(
                    GuardianSecretKeyShareGenerationError::InterGuardianShareCorrespondsToWrongPublicKey {
                        guardian_listed_in_interguardian_share,
                        guardian_listed_in_public_key,
                    },
                );
            }
        ...
        // Verify that there are no additional encrypted key sharess.
        if interguardian_shares.len() != guardian_public_keys.len() {
            return Err(GuardianSecretKeyShareGenerationError::GuardianPublicKeyAndInterGuardianShareQuantityMismatch {
                cnt_guardian_public_keys: guardian_public_keys.len(),
                cnt_interguardian_shares: interguardian_shares.len(),
            });
        }
        ...
        let mut shares = vec![];
        let mut issues = vec![];
        for (&sender_public_key, share) in zip(guardian_public_keys, interguardian_shares) {
...
mod t {
    ...
    fn test_encryption_decryption() {
        async_global_executor::block_on(async {
            ...
            let key_purpose = KeyPurpose::InterGuardianCommunication;
            ...
            /*
            let encrypted_result = InterguardianShare::encrypt(
                eg.csrng(),
                election_parameters,
                sk_sender,
                pk_recipient,
    ...
    fn test_key_sharing() {
        async_global_executor::block_on(async {
            ...
            let eg = {
                let mut config = EgConfig::new();
                config.use_insecure_deterministic_csprng_seed_str(
                    "eg::interguardian_share::test::test_key_sharing",
                );
                config.enable_test_data_generation_n_k(N, K).unwrap();
                Eg::from_config(config)
            };
            ...
            let share_vecs = gpks_rc
                ...
                .collect::<Vec<Vec<InterguardianShare>>>();
    ...
    fn test_public_validation() {
        async_global_executor::block_on(async {
            ...
            let eg = {
                let mut config = EgConfig::new();
                config.use_insecure_deterministic_csprng_seed_str(
                    "eg::interguardian_share::test::test_public_validation",
                );
                config.enable_test_data_generation_n_k(N, K).unwrap();
                Eg::from_config(config)
            };
            ...
            /*
            let enc_res_1 =
                InterguardianShare::encrypt(csrng, election_parameters, sk_one, pk_two)
                    .unwrap();

            let enc_res_2 =
                InterguardianShare::encrypt(csrng, election_parameters, sk_one, pk_one)
                    .unwrap();

            let enc_res_3 =
                InterguardianShare::encrypt(csrng, election_parameters, sk_two, pk_one)
                    .unwrap();
