...
//! For details, see:
//!
//! ElectionGuard Design Specification v2.1.0 sec 3.1.1 Standard Baseline Cryptographic Parameters
...
use crate::{
    ...
    fixed_parameters::{
        FixedParameterGenerationParameters, FixedParameters, FixedParametersInfo, NumsNumber,
    },
...
/// Standard parameters for the current version of the EG Design Specification
#[instrument(
    name = "eg::standard_parameters::make_standard_parameters",
    skip(produce_resource)
)]
pub fn make_standard_parameters(
    produce_resource: &(dyn ProduceResource + Send + Sync + 'static),
) -> EgResult<FixedParameters> {
    // The current version is EGDS 2.1
    let standard_parameters_info = make_egds_v2_1_standard_parameters_info();
    FixedParameters::try_validate_from(standard_parameters_info, produce_resource)
}

#[instrument(name = "eg::standard_parameters::make_standard_parameters_info")]
pub(crate) fn make_standard_parameters_info() -> FixedParametersInfo {
    // The current version is EGDS 2.1
    make_egds_v2_1_standard_parameters_info()
}

/// EGDS 2.1.0 sec 3.1.1 Standard Baseline Cryptographic Parameters pg. 14 eq. 3
#[instrument(
    name = "eg::standard_parameters::make_standard_parameters_egds_v2_1",
    skip(produce_resource)
)]
pub fn make_egds_v2_1_standard_parameters(
    produce_resource: &(dyn ProduceResource + Send + Sync + 'static),
) -> EgResult<FixedParameters> {
    let fixed_parameters_info = make_egds_v2_1_standard_parameters_info();
    FixedParameters::try_validate_from(fixed_parameters_info, produce_resource)
}

#[rustfmt::skip]
fn make_egds_v2_1_standard_parameters_info() -> FixedParametersInfo {
    let opt_egds_version = Some(crate::EGDS_RELEASED_V2_1_WITH_STANDARD_PARAMS.clone());
...
pub mod toy_parameters_do_not_use_in_production {
    ...
    pub fn make_toy_parameters_1() -> FixedParameters {
        ...
        let mut eg_design_specification_version =
            crate::EGDS_RELEASED_V2_1_WITH_STANDARD_PARAMS.clone();
        eg_design_specification_version.fixed_parameters_kind =
            crate::egds_version::ElectionGuard_FixedParameters_Kind::Toy_Parameters;

        let opt_egds_version = Some(eg_design_specification_version);
...
mod t {
    ...
    fn make_standard_parameters_egds_v2_1() -> EgResult<()> {
        ...
        assert!(matches!(
            standard_parameters.opt_egds_version(),
            Some(ElectionGuard_DesignSpecification_Version { version_number: [2, 1],
