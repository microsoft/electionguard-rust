...
//! This module provides the implementation of the [`JointPublicKey`] and [`Ciphertext`] for ballot
//! encryption. For more details see Sections `3.2.2` and `3.3` of the Electionguard specification
//! `2.0.0`. [TODO fix ref]
...
impl JointPublicKey {
    ...
    pub fn validate(&self, election_parameters: &ElectionParameters) -> EgResult<()> {
        ...
        if !valid {
            return Err(EgError::JointPublicKey_InvalidGroupElement(key_purpose));
        }
        Ok(())
    }
...
mod t {
    ...
    use crate::{
        eg::Eg, errors::EgResult, fixed_parameters::FixedParameters, guardian::GuardianKeyPurpose,
    ...
    #[test]
    pub fn scaling_ciphertext() -> EgResult<()> {
        use crate::guardian::GuardianKeyPurpose;
        let eg = Eg::new_with_test_data_generation_and_insecure_deterministic_csprng_seed(
            "eg::joint_public_key::t::scaling_ciphertext",
        );
        ...
        let joint_public_key = eg.joint_public_key(guardian_key_purpose)?;
